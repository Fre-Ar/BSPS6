...
def get_opts():
    ...
    # FKAN PE -- max frequency threshold Omega (Eq. 6 of Li et al. 2025).
    parser.add_argument('--omega', type=int, default=32,
                        help='Maximum frequency threshold Omega for FKAN PE (--pe FKAN).'
                             'PE output dim is 2 * in_features * Omega.')
    ...
    parser.add_argument('--seed', type=int, default=42,
                        help='Global random seed (preregistration §3.5). Default 42 '
    ...
    parser.add_argument('--runs_csv', type=str, default='results/runs.csv',
                        help='Path to append the per-run results row '
                             '(preregistration §3.6). Set to empty string to disable.')
    ...
    parser.add_argument('--rff_num_features', type=int, default=32,
                        help='Number of random Fourier features for '
                             '--ce spherical-rff (SRFF). Input dim = 2 * num_features. '
                             'Default 32 matches INR-Bench PE-RFF L=32 (preregistration §3.3).')
    parser.add_argument('--rff_sigma', type=float, default=10.0,
                        help='Frequency scale σ for spherical-RFF (SRFF) '
                             '(ω ~ N(0, σ² I_3) on cartesian coords). '
                             'Default 10.0 matches INR-Bench PE-RFF ffn_scale (preregistration §3.3).')
    ...
    parser.set_defaults(ce='angular')
    ...
    # Stash resolved kwargs for main.py to forward to SphericalDataset.
